...
export default function HistoryOrder() {
    ...
    return (
        <section className='py-8 px-4 sm:px-6 md:px-10 lg:px-16 xl:px-24 max-w-7xl mx-auto'>
            <div>
                <h1 className='text-3xl md:text-5xl font-medium flex gap-3'>
                    History Order <span className='text-xl bg-[#E8E8E8] p-3 pl-5 pr-5'>{pageInfo.totalData}</span>
                </h1>
            </div>
                                ...
                                <div key={index} className="flex flex-col md:flex-row gap-5 bg-[#E8E8E84D] p-5 rounded-md items-start md:items-center">
                                    <div className="shrink-0 hidden md:block">
                                        <div className="w-24 h-24 overflow-hidden rounded-lg bg-gray-200">
                                            <img
                                                src={order.image || order.image_products || Glass}
                                                alt="Food"
                                                className="w-full h-full object-cover"
                                                onError={(e) => { e.target.src = Glass; }}
                                            />
                                        </div>
                                    </div>
                                    ...
                                    <div className="grow grid grid-cols-2 sm:grid-cols-4 gap-4 w-full min-w-0">
                                        <div className="flex flex-col">
                                            ...
                                            <p className="font-bold mt-1 text-base">{(order.no_order || order.order_id || order.id || "-").toString().slice(0, 11)}</p>
                                            <button onClick={() => navigate(`/order/detail/${order.id || order.order_id}`)} className="text-brand-orange hover:text-[#ffad4e] text-sm font-medium underline text-left mt-1 hidden md:block">View Order Detail</button>
                                        </div>
                                        <div className="flex flex-col">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <p className="font-bold mt-1 text-base">{formatPrice(order.total || order.total_price || 0)}</p>
                                        </div>
                                        <div className="flex flex-col">
                                            <p className="flex gap-2 text-[#4F5665] text-sm"><img src={Process} alt="Icon" className="w-4 h-4" /> Status</p>
                                            <p className="font-semibold mt-1 bg-[#FF890633] text-brand-orange py-1 px-3 rounded-full text-xs w-fit capitalize">{order.status || "Pending"}</p>
                                        </div>
                                    </div>
                                    <button onClick={() => navigate(`/order/detail/${order.id || order.order_id}`)} className="text-brand-orange text-sm font-medium hover:underline text-left mt-1 block md:hidden">View Order Detail</button>
                                </div>
                            ))
                        )}
                    </div>
